...
static void SpawnExplosion(AppState *s, Vec2 pos, int count, float size_mult) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (rand() % 2 == 0) {
            s->particles[idx].color = (SDL_Color){255, (Uint8)(rand()%100+150), 50, 255}; 
        } else {
            s->particles[idx].color = (SDL_Color){(Uint8)(rand()%50+150), (Uint8)(rand()%50+150), (Uint8)(rand()%50+150), 255}; 
        }
        s->particle_next_idx = (s->particle_next_idx + 1) % MAX_PARTICLES;
    }
}
            ...
            if (dist_sq < min_dist * min_dist) {
                ...
                if (b.radius > 30.0f) {
                    ...
                    for (int k = 0; k < 2; k++) {
                        float split_angle = (k == 0) ? (collision_angle + 1.5708f) : (collision_angle - 1.5708f);
                        Vec2 dir = { cosf(split_angle), sinf(split_angle) };
                        SpawnAsteroid(s, (Vec2){b.pos.x + dir.x * child_rad, b.pos.y + dir.y * child_rad}, dir, child_rad);
                    }
                }
                break;
            }
