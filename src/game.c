...
void SpawnAsteroid(AppState *s, Vec2 pos, Vec2 vel_dir, float radius) {
  ...
  for (int i = 0; i < MAX_ASTEROIDS; i++) {
    if (!s->world.asteroids[i].active) {
      ...
      s->world.asteroids[i].rot_speed = ((float)(rand() % 100) / 50.0f - 1.0f) *
...
static void UpdateSimAnchors(AppState *s, Vec2 cam_center) {
  ...
  for (int i = 0; i < MAX_UNITS; i++) {
      ...
      if (s->world.sim_anchor_count >= MAX_SIM_ANCHORS) break;
      bool covered = false;
      ...
      if (!covered) s->world.sim_anchors[s->world.sim_anchor_count++].pos = s->world.units[i].pos;
  }
}

void Game_Init(AppState *s) {
    // Mothership Stats
    s->world.unit_stats[UNIT_MOTHERSHIP] = (UnitStats){
        .max_health = 1000.0f,
        .max_energy = 500.0f,
    ...
    // Scout Stats
    s->world.unit_stats[UNIT_SCOUT] = (UnitStats){
        .max_health = 200.0f,
        .max_energy = 100.0f,
        .speed = 1800.0f,
        .friction = 3.0f,
    ...
    for (int i = 0; i < MAX_UNITS; i++) s->world.units[i].active = false;
...
static void HandleRespawn(AppState *s, float dt, int win_w, int win_h) {
    if (s->ui.respawn_timer <= 0) return;
            ...
            if (s->world.units[i].type == UNIT_MOTHERSHIP) {
                ...
                while (attempts < RESPAWN_ATTEMPTS) {
                    ...
                    for (int j = 0; j < MAX_ASTEROIDS; j++) {
                        ...
                        if (Vector_DistanceSq((Vec2){rx, ry}, s->world.asteroids[j].pos) < powf(s->world.asteroids[j].radius + u->stats->radius + RESPAWN_BUFFER, 2)) { safe = false; break; }
                    }
...
static void UpdateSpawning(AppState *s, Vec2 cam_center) {
    ...
    for (int i = 0; i < MAX_ASTEROIDS; i++) {
        ...
        bool in_range = false;
        for (int a = 0; a < s->world.sim_anchor_count; a++) {
            if (Vector_DistanceSq(s->world.asteroids[i].pos, s->world.sim_anchors[a].pos) < DESPAWN_RANGE * DESPAWN_RANGE) { in_range = true; break; }
        }
        if (!in_range) { s->world.asteroids[i].active = false; s->world.asteroid_count--; }
    }

    int attempts = 0;
    while (s->world.asteroid_count < total_target_count && attempts < SPAWN_ATTEMPTS) {
        attempts++;
        ...
        if (((float)rand() / (float)RAND_MAX) < GetAsteroidDensity(spawn_pos)) {
            ...
            bool overlap = false;
            for (int j = 0; j < MAX_ASTEROIDS; j++) {
                if (s->world.asteroids[j].active && Vector_DistanceSq(s->world.asteroids[j].pos, spawn_pos) < powf((s->world.asteroids[j].radius + new_rad) * ASTEROID_HITBOX_MULT + SPAWN_BUFFER, 2)) { overlap = true; break; }
            }
            if (!overlap) {
...
static void UpdateRadar(AppState *s) {
    ...
    if (found && SDL_GetAtomicInt(&s->threads.radar_request_update) == 0) {
        SDL_LockMutex(s->threads.radar_mutex); s->threads.radar_mothership_pos = m_pos; SDL_UnlockMutex(s->threads.radar_mutex);
        SDL_SetAtomicInt(&s->threads.radar_request_update, 1);
    }
}

void Game_Update(AppState *s, float dt) {
  if (s->game_state == STATE_PAUSED) return;
  int win_w, win_h; SDL_GetRenderOutputSize(s->renderer, &win_w, &win_h);

  HandleRespawn(s, dt, win_w, win_h);
  if (s->ui.hold_flash_timer > 0) s->ui.hold_flash_timer -= dt;
  if (s->ui.auto_attack_flash_timer > 0) s->ui.auto_attack_flash_timer -= dt;
  if (s->ui.ui_error_timer > 0) s->ui.ui_error_timer -= dt;

  Vec2 cam_center = {s->camera.pos.x + (win_w / 2.0f) / s->camera.zoom, s->camera.pos.y + (win_h / 2.0f) / s->camera.zoom};
  float move_speed = CAMERA_SPEED / s->camera.zoom;
  if (s->input.mouse_pos.x < EDGE_SCROLL_THRESHOLD) s->camera.pos.x -= move_speed * dt;
  if (s->input.mouse_pos.x > (win_w - EDGE_SCROLL_THRESHOLD)) s->camera.pos.x += move_speed * dt;
  if (s->input.mouse_pos.y < EDGE_SCROLL_THRESHOLD) s->camera.pos.y -= move_speed * dt;
  if (s->input.mouse_pos.y > (win_h - EDGE_SCROLL_THRESHOLD)) s->camera.pos.y += move_speed * dt;
  ...
  for (int i = 0; i < MAX_UNITS; i++) {
      ...
      if (u->has_target) {
          ...
          if (cur_cmd->type == CMD_ATTACK_MOVE && cur_cmd->target_idx != -1 && !s->world.asteroids[cur_cmd->target_idx].active) {
          ...
          if (u->has_target) {
              ...
              if (dsq > UNIT_STOP_DIST * UNIT_STOP_DIST) { 
                  ...
                  if (!s->input.patrol_mode && (u->command_current_idx == u->command_count - 1) && dist < UNIT_BRAKING_DIST) speed *= (dist / UNIT_BRAKING_DIST);
      ...
      SDL_UnlockMutex(s->threads.unit_fx_mutex);
                          ...
                          if (l_target != -1 && s->world.asteroids[l_target].active) {
                              ...
                              float max_d = u->stats->main_cannon_range + s->world.asteroids[l_target].radius * ASTEROID_HITBOX_MULT;
                          ...
                          if (s->input.auto_attack_enabled) {
                              ...
                              for (int c = 0; c < 4; c++) if (s_targets[c] != -1 && s->world.asteroids[s_targets[c]].active) {

                                  s->world.asteroids[s_targets[c]].targeted = true;

                                  float dsq = Vector_DistanceSq(s->world.asteroids[s_targets[c]].pos, u->pos);

                                  float max_d = u->stats->small_cannon_range + s->world.asteroids[s_targets[c]].radius * ASTEROID_HITBOX_MULT;

                                  if (u->small_cannon_cooldown[c] <= 0 && dsq <= max_d * max_d) {
                  Weapons_Fire(s, u, s_targets[c], u->stats->small_cannon_damage, u->stats->small_cannon_energy_cost, false);
