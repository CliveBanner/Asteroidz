...
bool Persistence_SaveGame(const AppState *s, const char *filename) {
    ...
    fwrite(s->world.asteroids, sizeof(Asteroid), MAX_ASTEROIDS, f);
...
bool Persistence_LoadGame(AppState *s, const char *filename) {
    ...
    fread(s->world.asteroids, sizeof(Asteroid), MAX_ASTEROIDS, f);

    // Re-link pointers
    for (int i = 0; i < MAX_UNITS; i++) {
        if (s->world.units[i].active) {
            s->world.units[i].stats = &s->world.unit_stats[s->world.units[i].type];
        }
    }
