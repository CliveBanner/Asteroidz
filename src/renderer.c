...
static void DrawPlanetToBuffer(Uint32 *pixels, int size, float seed) {
  int center = size / 2;
  float radius =
      (size / 2.0f) * 0.4f; // Smaller planet to allow for huge atmosphere
  float atmo_outer = radius * 2.5f; // Atmosphere goes to 250% (fills texture)
  ...
  for (int y = 0; y < size; y++) {
    for (int x = 0; x < size; x++) {
      ...
      if (dist <= atmo_outer) {
        ...
        // Noise for surface and atmosphere
        // MULTI-OCTAVE PERLIN NOISE (FBM) - 3 octaves, favoring low frequency
        float noise = 0.0f;
        float amplitude = 0.9f;   // High initial impact for lower frequency
        float frequency = 0.015f; // Lower base frequency
        for (int o = 0; o < 3; o++) {
          noise += PerlinNoise2D((float)x * frequency + seed,
                                 (float)y * frequency + seed * 2.0f) *
                   amplitude;
          amplitude *= 0.3f; // Sharp drop-off for higher octaves
          frequency *= 2.0f;
        }
        ...
        if (dist > radius) {
          // ATMOSPHERE LAYER (Using Perlin Alpha)
          float atmo_t = (dist - radius) / (atmo_outer - radius);
          // REDUCED NOISE FREQUENCY for smoother atmosphere
          float atmo_noise =
              PerlinNoise2D((float)x * 0.01f + seed, (float)y * 0.01f);

          // Alpha falloff + Noise variation (Reduced Alpha)
          float alpha_f = (1.0f - atmo_t) * (0.2f + atmo_noise * 0.8f);
          alpha = (Uint8)(alpha_f * 80); // Much more subtle

          // Atmosphere color (Tinted towards Blue/Cyan)
          r = (Uint8)(r * 0.2f + 50 * alpha_f);
          g = (Uint8)(g * 0.2f + 150 * alpha_f);
          b = (Uint8)(b * 0.2f + 255 * alpha_f);
        }
...
static void GeneratePlanetTextures(AppState *s) {
  int p_size = 1024; // INCREASED RESOLUTION
...
static int SDLCALL BackgroundGenerationThread(void *data) {
  ...
  while (SDL_GetAtomicInt(&s->bg_should_quit) == 0) {
    if (SDL_GetAtomicInt(&s->bg_request_update) == 1) {
      ...
      float drift_macro_x = time * 0.05f, drift_macro_y = time * 0.03f,
            drift_detail_x = time * 0.15f, drift_detail_y = time * 0.1f;
      for (int i = 0; i < s->bg_w * s->bg_h; i++) {
        int x = i % s->bg_w, y = i / s->bg_w;
        float world_x = cam_pos.x + (x * BG_SCALE_FACTOR) / zoom,
              world_y = cam_pos.y + (y * BG_SCALE_FACTOR) / zoom;
        float n = ValueNoise2D(world_x * s_macro + drift_macro_x,
                               world_y * s_macro + drift_macro_y) *
                      w_macro +
                  ValueNoise2D(world_x * s_low + drift_detail_x,
                               world_y * s_low + drift_detail_y) *
                      w_low +
                  ValueNoise2D(world_x * s_med + drift_detail_x,
                               world_y * s_med + drift_detail_y) *
                      w_med;
        float variation = ValueNoise2D(world_x * color_scale + 12345.0f,
  ...
  do {                                                                         \
    ...
    count++;                                                                   \
            ...
            if (y < s->bg_h - 1)
              ADD_PIXEL(i + s->bg_w);
#undef ADD_PIXEL
            s->bg_pixel_buffer[i] = ((a_sum / count) << 24) |
                                    ((b_sum / count) << 16) |
                                    ((g_sum / count) << 8) | (r_sum / count);
          }
        }
      }
...
static void DrawInfiniteStars(SDL_Renderer *renderer, const AppState *s,
                              int win_w, int win_h) {
  int cell_size = 128;
  float parallax = 0.4f;
  float center_x = win_w / 2.0f, center_y = win_h / 2.0f;
  float cam_x_p =
      s->camera_pos.x * parallax + (center_x / s->zoom) * (parallax - 1.0f);
  float cam_y_p =
      s->camera_pos.y * parallax + (center_y / s->zoom) * (parallax - 1.0f);
  int start_gx = (int)floorf(cam_x_p / cell_size),
      start_gy = (int)floorf(cam_y_p / cell_size);
  int end_gx = (int)ceilf((cam_x_p + win_w / s->zoom) / cell_size),
      end_gy = (int)ceilf((cam_y_p + win_h / s->zoom) / cell_size);
    ...
    for (int gx = start_gx; gx <= end_gx; gx++) {
      ...
      if (seed > 0.85f) {
        float off_x = DeterministicHash(gx + 10, gy + 20) * cell_size,
              off_y = DeterministicHash(gx + 30, gy + 40) * cell_size;
        float drift_x = sinf(s->current_time * 0.2f + seed * 10.0f) * 15.0f,
              drift_y = cosf(s->current_time * 0.15f + seed * 5.0f) * 15.0f;
        float world_x = gx * cell_size + off_x + drift_x,
              world_y = gy * cell_size + off_y + drift_y;
        float screen_x = (world_x - cam_x_p) * s->zoom,
              screen_y = (world_y - cam_y_p) * s->zoom;
        float size_seed = DeterministicHash(gx + 55, gy + 66);
        float size = 1.0f;
        if (size_seed > 0.98f)
          size = 3.0f;
        else if (size_seed > 0.90f)
          size = 2.0f;
        float bright_seed = DeterministicHash(gx + 77, gy + 88);
        Uint8 color_val = (Uint8)(100 + bright_seed * 155),
              color_seed = DeterministicHash(gx + 99, gy + 11);
        ...
        if (s_size <= 1.0f) {
          ...
          SDL_FRect star_rect = {screen_x - s_size / 2.0f,
                                 screen_y - s_size / 2.0f, s_size, s_size};
...
static void DrawDistantPlanets(SDL_Renderer *renderer, const AppState *s,
                               int win_w, int win_h) {
  int cell_size = 5000;
  float parallax = 0.7f;
  float center_x = win_w / 2.0f, center_y = win_h / 2.0f;
  ...
  for (int gy = start_gy; gy <= end_gy; gy++) {
    for (int gx = start_gx; gx <= end_gx; gx++) {
      ...
      if (seed > 0.95f || is_origin) {
        float off_x = is_origin ? cell_size / 2
                                : DeterministicHash(gx + 5, gy + 9) * cell_size;
        float off_y = is_origin
                          ? cell_size / 2
                          : DeterministicHash(gx + 12, gy + 3) * cell_size;
        float world_x = gx * cell_size + off_x,
              world_y = gy * cell_size + off_y;
        float screen_x = (world_x - cam_x_p) * s->zoom,
              screen_y = (world_y - cam_y_p) * s->zoom;
        float radius_seed = DeterministicHash(gx + 7, gy + 11);
        float radius = (150.0f + radius_seed * 450.0f) * s->zoom;
        if (screen_x + radius < 0 || screen_x - radius > win_w ||
            screen_y + radius < 0 || screen_y - radius > win_h)
          continue;
        int planet_idx = (int)(DeterministicHash(gx + 1, gy + 1) * 8);
        float texture_size = radius * 2.5f;
        SDL_FRect dest = {screen_x - texture_size / 2,
                          screen_y - texture_size / 2, texture_size,
                          texture_size};
        SDL_RenderTexture(renderer, s->planet_textures[planet_idx], NULL,
                          &dest);
      }
    }
  }
...
static void DrawGrid(SDL_Renderer *renderer, const AppState *s, int win_w,
  ...
  for (float x = start_x_s; x < s->camera_pos.x + win_w / s->zoom + grid_small;
       x += grid_small) {
    float sx = (x - s->camera_pos.x) * s->zoom;
    SDL_RenderLine(renderer, sx, 0, sx, (float)win_h);
  }
  for (float y = start_y_s; y < s->camera_pos.y + win_h / s->zoom + grid_small;
       y += grid_small) {
  ...
  for (float x = start_x_l; x < s->camera_pos.x + win_w / s->zoom + grid_large;
       x += grid_large) {
    float sx = (x - s->camera_pos.x) * s->zoom;
    SDL_RenderLine(renderer, sx, 0, sx, (float)win_h);
  }
  for (float y = start_y_l; y < s->camera_pos.y + win_h / s->zoom + grid_large;
       y += grid_large) {
    float sy = (y - s->camera_pos.y) * s->zoom;
    SDL_RenderLine(renderer, 0, sy, (float)win_w, sy);
  }
  ...
  for (float x = start_x_l; x < s->camera_pos.x + win_w / s->zoom + grid_large;
       x += grid_large) {
    for (float y = start_y_l;
         y < s->camera_pos.y + win_h / s->zoom + grid_large; y += grid_large) {
      float sx = (x - s->camera_pos.x) * s->zoom,
            sy = (y - s->camera_pos.y) * s->zoom;
      if (sx >= -10 && sx < win_w && sy >= -10 && sy < win_h) {
        char label[32];
        snprintf(label, sizeof(label), "(%.0fk,%.0fk)", x / 1000.0f,
                 y / 1000.0f);
...
static void DrawDebugInfo(SDL_Renderer *renderer, const AppState *s,
                          int win_w) {
  char coords_text[64];
  snprintf(coords_text, sizeof(coords_text), "Cam: %.1f, %.1f (x%.4f)",
           s->camera_pos.x, s->camera_pos.y, s->zoom);
  SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
  float text_x =
      win_w - (SDL_strlen(coords_text) * SDL_DEBUG_TEXT_FONT_CHARACTER_SIZE) -
      20;
  float text_y = 20;
