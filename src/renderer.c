...
static void GetNebulaColor(float t, float *r, float *g, float *b) {
  static const ColorStop stops[] = {{0.00f, 2, 2, 6},
                                    {0.40f, 10, 10, 25},
                                    {0.70f, 25, 15, 45},
                                    {0.90f, 35, 60, 80},
                                    {1.00f, 50, 80, 100}};
  ...
  for (int y = 0; y < size; y++) {
    for (int x = 0; x < size; x++) {
      ...
      if (dist <= atmo_outer) {
        ...
        float nz = (dist <= radius)
                       ? sqrtf(fmaxf(0.0f, 1.0f - nx * nx - ny * ny))
                       : 0.0f;
        float noise = 0.0f, amp = 0.7f, freq = 0.03f;
        for (int o = 0; o < 3; o++) {
          noise += PerlinNoise2D((float)x * freq + seed,
                                 (float)y * freq + seed * 2.0f) *
                   amp;
          amp *= 0.3f;
          freq *= 2.0f;
        }
...
static void DrawGalaxyToBuffer(Uint32 *pixels, int size, float seed) {
  ...
  for (int y = 0; y < size; y++) {
    for (int x = 0; x < size; x++) {
      ...
      float arm_sin =
          powf(fabsf(cosf(twisted_angle * (float)arms / 2.0f)), arm_thickness);
      ...
      if (final_val > 0.02f) {
        ...
        float mix_val =
            fmaxf(0.0f, fminf(1.0f, (cn - 0.5f) * contrast_boost + 0.5f));
        ...
        Uint8 av =
            (Uint8)fminf(255, final_val * 255 * fminf(1.0f, falloff * 5.0f));
...
static void DrawAsteroidToBuffer(Uint32 *pixels, int size, float seed) {
  ...
  for (int y = 0; y < size; y++) {
    for (int x = 0; x < size; x++) {
      ...
      for (int o = 0; o < 3; o++) {
        shape_n += (PerlinNoise2D(cosf(angle) * s_freq + seed,
                                  sinf(angle) * s_freq + seed) -
                    0.5f) *
                   s_amp;
        s_freq *= 2.0f;
        s_amp *= 0.5f;
      ...
      if (dist <= distorted_radius) {
        ...
        float cracks1 =
            VoronoiCracks2D(x * 0.03f + seed + warp, y * 0.03f + seed + warp);
        float cracks2 =
            VoronoiCracks2D(x * 0.1f + seed * 2.0f, y * 0.1f + seed * 3.0f);
        float surface_v = ValueNoise2D(x * 0.05f + seed, y * 0.05f + seed);
        float mat_n =
            ValueNoise2D(x * 0.04f + seed * 4.0f, y * 0.04f + seed * 4.0f);
        ...
        Uint8 r =
            (Uint8)fminf(255, base_val * tr * soft_darken1 * soft_darken2);
        Uint8 g =
            (Uint8)fminf(255, base_val * tg * soft_darken1 * soft_darken2);
        Uint8 b =
            (Uint8)fminf(255, base_val * tb * soft_darken1 * soft_darken2);
        pixels[y * size + x] = (255 << 24) | (b << 16) | (g << 8) | r;
      } else {
        float dust_noise = PerlinNoise2D(cosf(angle) * 0.8f + seed + 100,
                                         sinf(angle) * 0.8f + seed + 100);
        float dust_outer = base_radius * (1.5f + dust_noise * 3.0f);
        if (dist <= dust_outer) {
          float dust_t =
              (dist - distorted_radius) / (dust_outer - distorted_radius);
          float detail_n =
              PerlinNoise2D(x * 0.02f + seed + 500, y * 0.02f + seed);
          float edge_dist =
              fminf(fminf(x, (size - 1) - x), fminf(y, (size - 1) - y));
          ...
          if (alpha_f > 0.01f) {
            ...
            pixels[y * size + x] =
                ((Uint8)(alpha_f * 100) << 24) | (val << 16) | (val << 8) | val;
          }
        }
      }
    }
  ...
  for (int y = 0; y < size; y++) {
    for (int x = 0; x < size; x++) {
      ...
      if (alpha_f > 0.05f) {
        ...
        pixels[y * size + x] =
            ((Uint8)(alpha_f * 255) << 24) | (val << 16) | (val << 8) | val;
      }
    }
  }
}
...
void Renderer_GenerateAssetStep(AppState *s) {
  ...
  if (s->assets_generated < PLANET_COUNT) {
    ...
    s->planet_textures[s->assets_generated] =
        SDL_CreateTexture(s->renderer, SDL_PIXELFORMAT_ABGR8888,
                          SDL_TEXTUREACCESS_STATIC, p_size, p_size);
    SDL_SetTextureBlendMode(s->planet_textures[s->assets_generated],
                            SDL_BLENDMODE_BLEND);
    SDL_UpdateTexture(s->planet_textures[s->assets_generated], NULL, pixels,
                      p_size * 4);
    SDL_free(pixels);
  } else if (s->assets_generated < PLANET_COUNT + GALAXY_COUNT) {
    int g_idx = s->assets_generated - PLANET_COUNT;
    int g_size = 1024;
    Uint32 *pixels = SDL_malloc(g_size * g_size * sizeof(Uint32));
    SDL_memset(pixels, 0, g_size * g_size * sizeof(Uint32));
    DrawGalaxyToBuffer(pixels, g_size, (float)g_idx * 123.45f + 99.0f);
    s->galaxy_textures[g_idx] =
        SDL_CreateTexture(s->renderer, SDL_PIXELFORMAT_ABGR8888,
                          SDL_TEXTUREACCESS_STATIC, g_size, g_size);
    ...
    SDL_free(pixels);
  } else if (s->assets_generated <
             PLANET_COUNT + GALAXY_COUNT + ASTEROID_TYPE_COUNT) {
    ...
    s->asteroid_textures[a_idx] =
        SDL_CreateTexture(s->renderer, SDL_PIXELFORMAT_ABGR8888,
                          SDL_TEXTUREACCESS_STATIC, a_size, a_size);
    ...
    int e_size = 256;
    Uint32 *pixels = SDL_malloc(e_size * e_size * sizeof(Uint32));
    SDL_memset(pixels, 0, e_size * e_size * sizeof(Uint32));
    DrawExplosionPuffToBuffer(pixels, e_size, 777.7f);
    s->explosion_puff_texture =
        SDL_CreateTexture(s->renderer, SDL_PIXELFORMAT_ABGR8888,
                          SDL_TEXTUREACCESS_STATIC, e_size, e_size);
...
void Renderer_DrawLoading(AppState *s) {
  ...
  float progress =
      (float)s->assets_generated /
      (float)(PLANET_COUNT + GALAXY_COUNT + ASTEROID_TYPE_COUNT + 1);
...
static int SDLCALL BackgroundGenerationThread(void *data) {
  ...
  while (SDL_GetAtomicInt(&s->bg_should_quit) == 0) {
    if (SDL_GetAtomicInt(&s->bg_request_update) == 1) {
      ...
      float s_macro = 0.0002f, s_low = 0.001f, s_med = 0.003f,
            color_scale = 0.00001f;
      float w_macro = 0.6f, w_low = 0.3f, w_med = 0.1f;
      float dmx = time * 0.05f, dmy = time * 0.03f, ddx = time * 0.15f,
            ddy = time * 0.1f;
      for (int i = 0; i < s->bg_w * s->bg_h; i++) {
        ...
        float n =
            ValueNoise2D(wx * s_macro + dmx, wy * s_macro + dmy) * w_macro +
            ValueNoise2D(wx * s_low + ddx, wy * s_low + ddy) * w_low +
            ValueNoise2D(wx * s_med + ddx, wy * s_med + ddy) * w_med;
        float variation = ValueNoise2D(wx * color_scale + 12345.0f,
                                       wy * color_scale + 67890.0f);
        if (n > 1.0f)
          n = 1.0f;
        float r_f, g_f, b_f;
        ...
        Uint8 r = (Uint8)fminf(r_f * intensity, 255.0f),
              g = (Uint8)fminf(g_f * intensity, 255.0f),
              b = (Uint8)fminf(b_f * intensity, 255.0f);
        float af = (n - 0.1f) / 0.6f;
        af = fmaxf(0.0f, fminf(1.0f, af));
      ...
      for (int pass = 0; pass < 1; pass++) {
        for (int y = 0; y < s->bg_h; y++) {
          for (int x = 0; x < s->bg_w; x++) {
            int i = y * s->bg_w + x;
            int rs = 0, gs = 0, bs = 0, as = 0, count = 0;
  ...
  do {                                                                         \
    ...
    count++;                                                                   \
            ...
            if (y < s->bg_h - 1)
              ADD_PIXEL(i + s->bg_w);
#undef ADD_PIXEL
            s->bg_pixel_buffer[i] = ((as / count) << 24) |
                                    ((bs / count) << 16) | ((gs / count) << 8) |
                                    (rs / count);
          }
        }
      }
  ...
  if (SDL_GetAtomicInt(&s->bg_data_ready) == 1) {
    ...
    if (SDL_LockTexture(s->bg_texture, NULL, &pixels, &pitch)) {
      ...
      for (int i = 0; i < s->bg_h; ++i)
        SDL_memcpy(dst + i * pitch, src + i * s->bg_w * 4, s->bg_w * 4);
...
static void StarLayerFn(SDL_Renderer *r, const AppState *s,
                        const LayerCell *cell) {
  if (cell->seed > 0.85f) {
    ...
    if (!IsVisible(sx, sy, s_sz / 2, cell->win_w, cell->win_h))
      return;

    float b_s = DeterministicHash(cell->gx + 77, cell->gy + 88),
          c_s = DeterministicHash(cell->gx + 99, cell->gy + 11);
    ...
    if (s_sz <= 1.0f)
      ...
      SDL_FRect rct = {sx - s_sz / 2, sy - s_sz / 2, s_sz, s_sz};
      SDL_RenderFillRect(r, &rct);
    }
  }
}

static void SystemLayerFn(SDL_Renderer *r, const AppState *s,
                          const LayerCell *cell) {
  Vec2 b_pos;
  float type_seed;
  float b_radius;
  if (GetCelestialBodyInfo(cell->gx, cell->gy, &b_pos, &type_seed, &b_radius)) {
    ...
    if (type_seed > 0.95f) {
      if (!IsVisible(sx, sy, rad, cell->win_w, cell->win_h))
        return;
      int g_idx =
          (int)(DeterministicHash(cell->gx + 9, cell->gy + 2) * GALAXY_COUNT);
      ...
      if (!IsVisible(sx, sy, rad, cell->win_w, cell->win_h))
        return;
      int p_idx =
          (int)(DeterministicHash(cell->gx + 1, cell->gy + 1) * PLANET_COUNT);
      float t_sz = rad * 4.5f;
      SDL_RenderTexture(r, s->planet_textures[p_idx], NULL,
                        &(SDL_FRect){sx - t_sz / 2, sy - t_sz / 2, t_sz, t_sz});
    }
  }
}

static void Renderer_DrawAsteroids(SDL_Renderer *r, const AppState *s,
                                   int win_w, int win_h) {
  for (int i = 0; i < MAX_ASTEROIDS; i++) {
    if (!s->asteroids[i].active)
      continue;
    Vec2 sx_y =
        WorldToScreenParallax(s->asteroids[i].pos, 1.0f, s, win_w, win_h);
    float rad = s->asteroids[i].radius * s->zoom;
    if (!IsVisible(sx_y.x, sx_y.y, rad * 1.5f, win_w, win_h))
      continue;
    SDL_RenderTextureRotated(
        r, s->asteroid_textures[s->asteroids[i].tex_idx], NULL,
        &(SDL_FRect){sx_y.x - rad * 1.5f, sx_y.y - rad * 1.5f, rad * 3.0f,
                     rad * 3.0f},
        s->asteroids[i].rotation, NULL, SDL_FLIP_NONE);
  }
}

static void Renderer_DrawParticles(SDL_Renderer *r, const AppState *s,
                                   int win_w, int win_h) {
  for (int i = 0; i < MAX_PARTICLES; i++) {
    if (!s->particles[i].active)
      continue;
    Vec2 sx_y =
        WorldToScreenParallax(s->particles[i].pos, 1.0f, s, win_w, win_h);
    float sz = s->particles[i].size * s->zoom;
    if (!IsVisible(sx_y.x, sx_y.y, sz, win_w, win_h))
      continue;
    if (s->particles[i].type == PARTICLE_PUFF && s->explosion_puff_texture) {
      SDL_SetTextureColorMod(s->explosion_puff_texture, s->particles[i].color.r,
                             s->particles[i].color.g, s->particles[i].color.b);
      SDL_SetTextureAlphaMod(s->explosion_puff_texture,
                             (Uint8)(s->particles[i].life * 180));
      SDL_RenderTexture(r, s->explosion_puff_texture, NULL,
                        &(SDL_FRect){sx_y.x - sz / 2, sx_y.y - sz / 2, sz, sz});
    } else {
      SDL_SetRenderDrawColor(r, s->particles[i].color.r,
                             s->particles[i].color.g, s->particles[i].color.b,
                             (Uint8)(s->particles[i].life * 255));
      SDL_RenderFillRect(
          r, &(SDL_FRect){sx_y.x - sz / 2, sx_y.y - sz / 2, sz, sz});
    }
  }
}
...
static void DrawMinimap(SDL_Renderer *r, const AppState *s, int win_w,
                        int win_h) {
  float mm_x = (float)win_w - MINIMAP_SIZE - MINIMAP_MARGIN;
  float mm_y = (float)win_h - MINIMAP_SIZE - MINIMAP_MARGIN;
      ...
      if (GetCelestialBodyInfo(gx, gy, &b_pos, &type_seed, &b_radius)) {
        ...
        if (fabsf(dx) < MINIMAP_RANGE / 2 && fabsf(dy) < MINIMAP_RANGE / 2) {
          ...
          SDL_RenderFillRect(r,
                             &(SDL_FRect){px - dot_size / 2, py - dot_size / 2,
                                          dot_size, dot_size});
        }
      }
    }
  }
  float view_w = ((float)win_w / s->zoom) * world_to_mm;
  float view_h = ((float)win_h / s->zoom) * world_to_mm;
  SDL_SetRenderDrawColor(r, 255, 255, 255, 255);
  SDL_RenderRect(r, &(SDL_FRect){mm_x + (MINIMAP_SIZE - view_w) / 2,
                                 mm_y + (MINIMAP_SIZE - view_h) / 2, view_w,
                                 view_h});
}
