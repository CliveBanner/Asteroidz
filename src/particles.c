...
void Particles_SpawnExplosion(AppState *s, Vec2 pos, int count, float size_mult, ExplosionType type, int asteroid_tex_idx) {
  float capped_mult = powf(size_mult, 0.7f) * 1.5f; // Increased scaling and base size
  float count_mult = powf(size_mult, 0.5f) * 2.0f; // More particles for larger asteroids
  float chunky_mult = powf(size_mult, 0.8f) * 1.8f;
  ...
  if (type == EXPLOSION_IMPACT) {
      int spark_count = (int)(count * 4.0f * count_mult); // Increased from 2.5
      for (int i = 0; i < spark_count; i++) {
        ...
        float speed = (float)(rand() % 1000 + 500) * capped_mult; 
        s->world.particles.velocity[idx].x = cosf(angle) * speed;
        s->world.particles.velocity[idx].y = sinf(angle) * speed;
        s->world.particles.life[idx] = PARTICLE_LIFE_BASE * 0.6f;
        s->world.particles.size[idx] = (float)(rand() % 16 + 8) * capped_mult;
      ...
      int puff_count = (int)(count * 2.0f * count_mult); // Increased from 1.2
      for (int i = 0; i < puff_count; i++) {
        ...
        s->world.particles.pos[idx] = pos;
        float angle = (float)(rand() % 360) * 0.0174533f;
        float speed = (float)(rand() % 250 + 100) * capped_mult; 
        s->world.particles.velocity[idx].x = cosf(angle) * speed;
        s->world.particles.velocity[idx].y = sinf(angle) * speed;
        s->world.particles.life[idx] = PARTICLE_LIFE_BASE * 0.6f; 
        s->world.particles.size[idx] = (float)(rand() % 180 + 100) * capped_mult;
        Uint8 v = (Uint8)(rand() % 40 + 80); 
        s->world.particles.color[idx] = (SDL_Color){v, (Uint8)(v * 0.8f), (Uint8)(v * 0.6f), 255};
        s->world.particle_next_idx = (s->world.particle_next_idx + 1) % MAX_PARTICLES;
      }
      int fine_debris_count = (int)(20 * count_mult); // Doubled
      for (int i = 0; i < fine_debris_count; i++) {
          ...
          float angle = (float)(rand() % 360) * 0.0174533f;
          float speed = (float)(rand() % 400 + 150) * capped_mult; 
          s->world.particles.velocity[idx].x = cosf(angle) * speed;
          s->world.particles.velocity[idx].y = sinf(angle) * speed;
          s->world.particles.life[idx] = PARTICLE_LIFE_BASE * 0.45f; 
          s->world.particles.size[idx] = (float)(rand() % 24 + 16) * chunky_mult;
          s->world.particles.rotation[idx] = (float)(rand() % 360);
          s->world.particles.color[idx] = base_col;
          s->world.particle_next_idx = (s->world.particle_next_idx + 1) % MAX_PARTICLES;
      }
      int debris_count = (int)(8 * count_mult); // Increased from 3
      for (int i = 0; i < debris_count; i++) {
          ...
          s->world.particles.velocity[idx].x = cosf(angle) * speed;
          s->world.particles.velocity[idx].y = sinf(angle) * speed;
          s->world.particles.life[idx] = PARTICLE_LIFE_BASE * 0.55f; 
          s->world.particles.size[idx] = (float)(rand() % 60 + 40) * chunky_mult;
          s->world.particles.rotation[idx] = (float)(rand() % 360);
          s->world.particles.color[idx] = base_col;
          s->world.particle_next_idx = (s->world.particle_next_idx + 1) % MAX_PARTICLES;
      }
      ...
      s->world.particles.size[sw_idx] = 100.0f * capped_mult; // Larger shockwave
      s->world.particles.color[sw_idx] = (SDL_Color){255, 255, 255, 200}; 
      s->world.particle_next_idx = (s->world.particle_next_idx + 1) % MAX_PARTICLES;
  } else {
      int puff_count = (int)(count * 1.5f * count_mult); // Increased from 0.5
      for (int i = 0; i < puff_count; i++) {
        int idx = s->world.particle_next_idx;
        s->world.particles.active[idx] = true;
        s->world.particles.type[idx] = PARTICLE_PUFF;
      ...
      s->world.particles.pos[sw_idx] = pos;
      s->world.particles.velocity[sw_idx] = (Vec2){0,0};
      s->world.particles.life[sw_idx] = 0.7f;
      s->world.particles.size[sw_idx] = (float)(150.0f * capped_mult); 
